...
#if UNIFORM_LOGGING
#define GE_LOG_UNIFORM(...) GE_CORE_WARN(__VA_ARGS__)
#else
#define GE_LOG_UNIFORM
#endif
	...
	std::string OpenGLShader::ReadShaderFromFile(const std::string& filepath) const
	{
		...
		{
			GE_CORE_ASSERT(false, "Could not load shader!");
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			GE_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
	{
		...
		GE_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
		...
		case OpenGLShaderUniformDeclaration::Type::Struct:
			...
			break;
		default:
			GE_CORE_ASSERT(false, "Unknown uniform type!");
		}
	}
	...
	void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
	{
		...
		if (location != -1)
			...
			GE_LOG_UNIFORM("Uniform '{0}' not found!", name);
	}
