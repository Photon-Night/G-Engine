...
namespace GEngine
{
	static GLenum TextureFormatToOpenGLTextureFormat(TextureFormat format)
	{
		switch (format)
		{
		case GEngine::TextureFormat::RGB:     return GL_RGB;
		case GEngine::TextureFormat::RGBA:    return GL_RGBA;
		case GEngine::TextureFormat::Float16: return GL_RGBA16F;
		}
		GE_CORE_ASSERT(false, "Unknown texture format!");
		return 0;
	}
		...
		Renderer::Submit([this]()
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, TextureFormatToOpenGLTextureFormat(m_Format), m_Width, m_Height, 0, TextureFormatToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		{
			GE_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		{
			GE_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			GE_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		Renderer::Submit([this]() {
			glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, TextureFormatToOpenGLTextureFormat(m_Format), GL_UNSIGNED_BYTE, m_ImageData.Data);
			});
	}
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([=]() {
			...
			glTextureStorage2D(m_RendererID, levels, TextureFormatToOpenGLTextureFormat(m_Format), width, height);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, levels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
			glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		...
		: m_FilePath(path)
	{
		...
		GE_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
