...
static void FindHoveredWindow()
{
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        // Using the clipped BoundingBox, a child window will typically be clipped by its parent (not always)
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            // We build a throwaway ID based on current ID stack + relative BoundingBox of items in window.
